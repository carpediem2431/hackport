...
export function CampClient() {
  ...
  // Read search params after mount to avoid hydration mismatch
  useEffect(() => {
    if (initialHackathon !== "all") {
      setSelectedHackathon(initialHackathon);
    }
    setMounted(true);
  }, [initialHackathon]);

  // Show demo data only when localStorage hasn't been initialized yet (not ready)
  const teams = teamsReady ? storedTeams : demoTeamPosts;
