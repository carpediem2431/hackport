...
function areChecklistStatesEqual(left: Record<string, boolean>, right: Record<string, boolean>) {
...
function isSameProgressState(previous: UserProgress | undefined, next: Omit<UserProgress, "updatedAt">) {
  if (!previous) {
    return false;
  }

  return (
    previous.hasTeam === next.hasTeam
    && previous.hasDraft === next.hasDraft
    && previous.isSubmitted === next.isSubmitted
    && previous.viewedSections.length === next.viewedSections.length
    && previous.viewedSections.every((section, index) => section === next.viewedSections[index])
    && areChecklistStatesEqual(previous.checklistState, next.checklistState)
  );
}
...
export function SubmissionPanel({ hackathon }: { hackathon: Hackathon }) {
  ...
  useEffect(() => {
    ...
    const p = progress;
    const nextProgress = {
      viewedSections: p?.viewedSections ?? [],
    ...
    if (isSameProgressState(p, nextProgress)) {
      return;
    }
  ...
  const updateDraft = (next: Partial<SubmissionDraft>) => {
    setSubmissions((previous) => ({
      ...previous,
      [hackathon.slug]: {
        ...(previous[hackathon.slug] ?? emptySubmission),
        ...next,
      },
    }));
  };
    ...
    <Card className="bg-white">
      ...
      <div className="mt-6 space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-[24px] border border-border bg-[#f7f3eb] p-5">
              ...
              <div className="mt-4 grid gap-2 text-sm text-muted">
                ...
                <div>증빙 노트: {draft.notes ? "입력됨" : "미입력"}</div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <Textarea className="min-h-[140px]" placeholder="비공개 메모: 로컬에서만 저장됩니다" value={draft.privateMemo} onChange={(event) => updateDraft({ privateMemo: event.target.value })} />
        </div>
      ...
      >
        <div className="space-y-4">
          ...
          {!validation.isValid ? (
            <div className="rounded-[20px] border border-danger/20 bg-danger/10 p-4 text-sm text-danger">
              {validation.issues.map((issue) => (
                <p key={issue}>{issue}</p>
              ))}
            </div>
